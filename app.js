...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ==========================================================================
  // 1. Secure Portal Preloader Screen Setup (CLI Startup Animation)
  // ==========================================================================
  const preloader = document.getElementById("preloader");
  const loaderOutput = document.getElementById("loader-output");

  const logs = [
    { text: '> INITIALIZING SECURE LINK PROTOCOL...', isCmd: true },
    { text: '[+] SYSTEM STATUS CHECK: OK', isCmd: false },
    { text: '> LOADING USER CONFIGURATION REGISTRY...', isCmd: true },
    { text: '[+] CONFIGURATION REGISTRY LOADED SUCCESSFUL', isCmd: false },
    { text: '> VERIFYING PGP SIGNATURES FOR NODE [MANAS]...', isCmd: true },
    { text: '[+] KEY VALIDATED SUCCESSFUL [0x2A8C4B9F]', isCmd: false },
    { text: '> DECRYPTING SYSTEM SCHEMAS AND ASSETS...', isCmd: true },
    { text: '[+] SYSTEM DECRYPTION SUCCESSFUL', isCmd: false },
    { text: '> ESTABLISHING SECURE PROTOCOLS ON PORT 8080...', isCmd: true },
    { text: '[+] SECURE TRUNK PROTOCOL STACK STARTED', isCmd: false },
    { text: '> LAUNCHING COMPONENT CONTROLLER INTERFACE...', isCmd: true },
    { text: '[+] INTERFACE STANDBY. READY.', isCmd: false }
  ];

  let logIndex = 0;
  function printLog() {
    if (!preloader || !loaderOutput) return;
    if (logIndex < logs.length) {
      const item = logs[logIndex];
      const p = document.createElement('p');
      p.className = item.isCmd ? 'loader-log-cmd' : 'loader-log-ok';
      p.textContent = item.text;
      loaderOutput.appendChild(p);
      loaderOutput.scrollTop = loaderOutput.scrollHeight;
      logIndex++;
      setTimeout(printLog, 80 + Math.random() * 80);
    } else {
      setTimeout(() => {
